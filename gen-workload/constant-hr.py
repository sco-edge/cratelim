...
async def send_http_request(session, port, parameters, headers):
    try:
        ...
        async with session.get(f"http://localhost:{port}/hotels", params=parameters, headers=headers) as response:
            ...
            results.append((datetime.fromtimestamp(start_time, tz=timezone.utc).isoformat(), response_time, response.status))
...
async def http_test_with_rps(rps_list, duration, port, params, headers):
    ...
    if results:
        ...
        print(f"{len(results)} {failed_responses} {np.mean(response_times)} {np.percentile(response_times, 50)}",
...
asyncio.run(http_test(rps, len(rps), "8080", params, headers))

with open('constant.log', 'w') as file:
    for result in results:
        json.dump({"time": result[0], "latency": result[1], "response_code": result[2]}, file)
        file.write("\n")
